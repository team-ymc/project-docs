...
- 플랜별 AI 답변 횟수의 판정·예약·확정 → FT-011 플랜·사용량 제한
...
> **Resolved** — AI 튜터 채팅의 플랜별 횟수 제한은 FT-011이 소유한다. 플랜별 N과 적용 기간은 정책값으로 정하고, 기존 ASSISTANT row에 최종 답변이 저장되어 `COMPLETED` 전이가 commit된 경우를 1회 사용 완료로 확정한다. 유한한 정책의 예약·확정·해제 의미론도 FT-011을 따른다.
