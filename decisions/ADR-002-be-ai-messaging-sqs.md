...
- Implements: `contracts/asyncapi.yaml`, `contracts/schema/parse-request.schema.json`, `contracts/schema/parse-result.schema.json`
...
- 직접 만들면 리스 (일종의 Lock)· 회수 배치(멈춘 작업을 되돌림) · DLQ · 모니터링의 관리 소요가 발생한다.
...
- 단점: **visibility timeout을 최대 파싱 시간보다 길게 튜닝해야 한다.
	- ** 짧으면 처리 중 재전달 → 중복 파싱이 난다. 현재 로컬은 900초(`infra/local/bootstrap.sh`)이며, 파싱이 이를 넘길 수 있게 되면 `ChangeMessageVisibility` heartbeat를 검토한다.
- 단점: at-least-once다. 중복 수신 방어(멱등성)를 고려해야 한다.
...
### Option D. 직접 호스팅하는 메시지 브로커 (RabbitMQ · Kafka)

- 장점: 기능이 풍부하다. RabbitMQ는 라우팅 · 우선순위 · 순서 보장을, Kafka는 높은 처리량 · 로그 재처리 · 다중 컨슈머 그룹을 준다.
- 단점: **상태를 가진 브로커를 직접 운영해야 한다** — 영속성, 클러스터링, 장애 복구, 모니터링.
- 탈락 사유: **운영할 사람이 없다.**  매니지드(Amazon MQ · MSK)로 운영을 덜 수 있으나 SQS보다 비싸고 개념 부담이 크면서 얻는 것이 없다.
...
- **멱등성을 계약에 명시한다.** `contracts/asyncapi.yaml`에 "`parse-result`는 at-least-once이며, 소비자는 멱등해야 한다"를 적고, BE의 조건부 전이가 그 구현임을 `ParseResultListener`에 남긴다. 지금은 우연히 막고 있는 것처럼 읽힌다.
