...
"발행하지 않음"에는 예외가 하나 있다. 기존 `Document`가 발행 실패나 중단으로 아직 파싱 발행 전
상태에 정체돼 있으면, 이후의 아무 complete(연결 경로와 멱등 재호출 포함)가 시작 권한을 선점해
발행을 구제할 수 있다. 이때도 메시지 식별자는 `Document`에 보존된 최초 `paperId`를 사용한다.

시작 권한 선점(조건부 갱신)은 발행에 선행한다. 여러 `Paper`의 complete가 같은 `Document`에
닿으므로 전이를 먼저 확정해야 이중 발행이 구조적으로 차단된다 — 발행 주체가 경로상 하나였던
Paper 단위 시퀀스(ADR-002 §4, 발행 후 전이)와 의도적으로 다르다.
...
- **2026-08-12** — 발행 전 상태로 정체된 `Document`의 구제 발행 예외와, 시작 권한 선점이
  발행에 선행함(ADR-002 시퀀스와의 의도적 편차)을 명시. BE 구현 스펙(YMC-280) 리뷰에서 도출.
